...
// PATCH  /api/users?id=xxx   — update user (admin: any field; user: own password only)
// DELETE /api/users?id=xxx   — deactivate user (admin only)
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      await sql`INSERT INTO users (id, username, password_hash, display_name, role, created_by)
  ...
  if (req.method === 'PATCH') {
    ...
    if (payload.role === 'admin') {
      if (username !== undefined) {
        ...
        try {
          await sql`UPDATE users SET username = ${clean} WHERE id = ${id}`;
        } catch (err) {
          if (err.message.includes('unique')) return res.status(409).json({ error: 'Username already taken' });
          throw err;
        }
  ...
  // DELETE — deactivate (soft delete) — admin only
